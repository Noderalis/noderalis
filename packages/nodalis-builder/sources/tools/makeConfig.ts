import { Configuration, identity } from '@nodalis/core';
...
export class WebpackConfiguration {
	...
	async create(): Promise<WebpackConfigurationDefinition[]> {
		...
		const configs = [
			merge(baseConfig, await this.createEntry('@nodalis/builder')),
			merge(baseConfig, await this.createEntry('@nodalis/cli')),
			merge(baseConfig, await this.createEntry('@nodalis/core')),
		];

...
import {
	isEngramPrintedGenTyping,
	isEngramPrintedGenTypingImport,
...
export class TypegenPrinter {
	...
	printDynamicImport() {
		...
		} = this.schema.extensions.engram.config;
		const imports: string[] = [];
		const importMap: Record<string, Set<string>> = {};
		const outputPath = this.typegenInfo.typegenFile;
		const engramSchemaImportId =
			this.typegenInfo.engramSchemaImportId ?? getOwnPackage().name;
	...
	printDynamicInputFieldDefinitions() {
		const {
			dynamicInputFields,
		} = this.schema.extensions.engram.config.dynamicFields;
	...
	printAbstractResolveReturnTypeMap() {
		return this.printTypeInterface(
			'EngramGenAbstractResolveReturnTypes',
	...
	resolveBackingType(typeName: string): string | undefined {
		const rootTyping = this.schema.extensions.engram.config.rootTypings[
			typeName
		];
		if (rootTyping) {
	...
	printType(strLike: StringLike | StringLike[]): string {
		...
		if (isEngramPrintedGenTyping(strLike)) {
			strLike.imports.forEach((i) => {
				this.addImport(i);
			});
			return strLike.toString();
		}
		if (isEngramPrintedGenTypingImport(strLike)) {
	...
	addImport(i: PrintedGenTypingImport) {
		/* istanbul ignore if */
		if (!isEngramPrintedGenTypingImport(i)) {

...
export class TypegenMetadata {
	 ...
	 */
	async generateArtifacts(schema: EngramGraphQLSchema) {
	...
	sortSchema(schema: EngramGraphQLSchema) {
		let sortedSchema = schema;
		if (typeof lexicographicSortSchema !== 'undefined') {
			sortedSchema = lexicographicSortSchema(schema) as EngramGraphQLSchema;
		}
		return sortedSchema;
	}
	...
	async writeFile(type: 'schema' | 'types', output: string, filePath: string) {
		if (typeof filePath !== 'string' || !path.isAbsolute(filePath)) {
			return Promise.reject(
				new Error(
					`Expected an absolute path to output the Engram ${type}, saw ${filePath}`
				)
			);
		}

...
exports.nullabilityGuardPlugin = (pluginConfig) => {
    ...
    return plugin_1.plugin({
        ...
        onCreateFieldResolver(config) {
            var _a, _b;
            if ((_b = (_a = config.fieldConfig.extensions) === null || _a === void 0 ? void 0 : _a.engram) === null || _b === void 0 ? void 0 : _b.config.skipNullGuard) {
                return;
            }
            ...
            if (outerNonNull || hasListNonNull) {
                return (root, args, ctx, info, next) => {
                    return plugin_1.plugin.completeValue(next(root, args, ctx, info), nonNullGuard(ctx, info, graphql_1.isNonNullType(type) ? type.ofType : type, config, finalPluginConfig, outerNonNull));
                };
            }
            return;
        },
...
const nonNullGuard = (ctx, info, type, config, pluginConfig, outerNonNull) => {
    ...
    return (val) => {
        ...
        if (graphql_1.isListType(type)) {
            ...
            const listMembers = [];
            const listCompleter = nonNullGuard(ctx, info, graphql_1.isNonNullType(type.ofType) ? type.ofType.ofType : type.ofType, config, pluginConfig, graphql_1.isNonNullType(type.ofType));
            iterall_1.forEach(val, (item) => {
                if (!hasPromise && utils_1.isPromiseLike(item)) {
                    hasPromise = true;
                }

    ...
    return plugin_1.plugin({
        ...
        fieldDefTypes: [
            utils_1.printedGenTypingImport({
                module: (_a = connectionPluginConfig === null || connectionPluginConfig === void 0 ? void 0 : connectionPluginConfig.engramSchemaImportId) !== null && _a !== void 0 ? _a : utils_1.getOwnPackage().name,
        ...
        onInstall(b) {
            let dynamicConfig = [];
            const { additionalArgs = {}, extendConnection: pluginExtendConnection, extendEdge: pluginExtendEdge, includeNodesField = false, engramFieldName = 'connectionField', } = pluginConfig;
            ...
            config: connectionPluginCore.ConnectionFieldConfig<TypeName, FieldName> ${printedDynamicConfig}
                ...
                factory({ typeName: parentTypeName, typeDef: t, args: factoryArgs, stage, }) {
                    ...
                    const { disableBackwardPagination, disableForwardPagination, validateArgs = defaultValidateArgs, strictArgs = true, } = {
                        ...pluginConfig,
                        ...fieldConfig,
                    };
                    let specArgs = {};
...
function nonConnectionFieldProps(fieldConfig) {
    const { additionalArgs, cursorFromNode, disableBackwardPagination, disableForwardPagination, extendConnection, extendEdge, inheritAdditionalArgs, nodes, pageInfoFromNodes, resolve, type, validateArgs, strictArgs, ...rest } = fieldConfig;
    return rest;
}
...
const assertCorrectConfig = (typeName, fieldName, pluginConfig, fieldConfig) => {
    if (typeof fieldConfig.nodes !== 'function' &&
        ...
        console.error(new Error(`Engram Connection Plugin: Missing nodes or resolve property for ${typeName}.${fieldName}`));
    }
    utils_1.eachObj(pluginConfig.extendConnection || {}, (val, key) => {
        if (typeof fieldConfig[key] !== 'function') {
            console.error(new Error(`Engram Connection Plugin: Missing ${key} resolver property for ${typeName}.${fieldName}`));
        }
    });
    utils_1.eachObj(pluginConfig.extendEdge || {}, (val, key) => {
        if (!utils_1.isObject(fieldConfig.edgeFields) ||
            ...
            console.error(new Error(`Engram Connection Plugin: Missing edgeFields.${key} resolver property for ${typeName}.${fieldName}`));
        }
    });
};

...
import { NodalisPlugin } from './NodalisPlugin';
...
const ENVIRONMENT_PREFIX = `nodalis_`;
...
export type NodalisPluginConfiguration = {
...
export class Configuration {
	...
	private static async findNodalisFiles(startingCwd: string) {
		let nodalisFiles: { path: string; cwd: string; data: any }[] = [];
		...
		while (nextCwd !== currentCwd) {
			...
			const nodalisPath = join(currentCwd, '.nodalis.yml');

			if (existsSync(nodalisPath)) {
				const content = await readFileSync(nodalisPath, 'utf8');
				let data;
				...
				try {
					...
					throw new UsageError(
						`Parse error when loading ${nodalisPath}. Please check that it's proper Yaml!`
					);
				}
	...
	private static async findProjectCwd(
		...
		while (nextCwd !== currentCwd) {
			...
			if (nodalisFile !== null) {
				if (existsSync(join(currentCwd, nodalisFile))) {
	...
	public static async find(
		startingCwd: string,
		pluginConfiguration: NodalisPluginConfiguration | null,
		{ lookup = ProjectLookup.NODALIS, strict = true } = {}
	) {
		console.log(strict);
		...
		const plugins = new Map<string, NodalisPlugin>();
	...
	public plugins: Map<string, NodalisPlugin> = new Map();
	...
	private activatePlugin(name: string, plugin: NodalisPlugin) {
			...
			for (const key of Object.keys(data)) {
				...
				if (!definition) {
					if (strict) {
						...
						throw new UsageError(
							`Unrecognized configuration settings found: "${key}"\nrun "yarn config -v" to see the list of settings supported in Nodalis`
						);
					} else {

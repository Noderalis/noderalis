...
const requireAbs = (module_1.createRequire || module_1.createRequireFromPath)(
	path_1.default.resolve(__dirname, './index.ts')
);
class ProjectConfiguration extends AbstractConfiguration_1.AbstractConfiguration {
	...
	/**
	 * Creates a the Nodalis configuration. Called from within `.nodalis.ts` only.
	 * @param config
	 */
			...
			parsedPkg.workspaces.forEach((workspaceDir) => {
				if (workspaceDir.includes('*')) {
					...
					fs_1.default.readdir(
						path_1.default.join(rootDir, workspace),
						async (err, files) => {
							console.log(
								'2:rootDir ' + path_1.default.join(rootDir, workspace)
							);
							if (err) throw console.log(`Error: ${err}. Files: ${files}`);
							for (let i = 0; i < files.length; i++) {
								const testDir = path_1.default.join(
									rootDir,
									workspace,
									files[i]
								);
								console.log(
									'3:rootDir ' +
										path_1.default.join(rootDir, workspace, files[i])
								);
								currentCwd = testDir;
								console.log(`package location: ${currentCwd}\\package.json`);
								console.log(`desired name: ${workspaceName}`);
								const workspacePkg = await JSON.parse(
									fs_1.default.readFileSync(`${currentCwd}/package.json`, {
										encoding: 'utf-8',
									})
								);
								...
								if (workspaceName == workspacePkg.name) {
									resolve(
										path_1.default.resolve(currentCwd, workspacePkg.main)
									);
								}
							}
	...
	static async findProjectCwd(startingCwd, nodalis = false) {
		...
		while (nextCwd !== currentCwd) {
			currentCwd = nextCwd;
			if (
				fs_1.default.existsSync(path_1.default.join(currentCwd, 'package.json'))
			) {
			...
			if (nodalis) {
				if (
					fs_1.default.existsSync(
						path_1.default.join(currentCwd, 'nodalis.json')
					)
				) {
					console.log('found nodalis' + currentCwd);
					projectCwd = currentCwd;
					break;
				}
			} else {
...
async function testWrkspc() {
	const builder = await ProjectConfiguration.workspace('@nodalis/builder');

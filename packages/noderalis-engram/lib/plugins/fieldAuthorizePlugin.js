...
exports.defaultFormatError = ({ error, }) => {
    const err = new Error('Not authorized');
    err.originalError = error;
    return err;
};
exports.fieldAuthorizePlugin = (authConfig = {}) => {
    ...
    return plugin_1.plugin({
        name: 'EngramAuthorize',
        description: 'The authorize plugin provides field-level authorization for a schema.',
        fieldDefTypes: fieldDefTypes,
        onCreateFieldResolver(config) {
            var _a, _b, _c;
            const authorize = (_b = (_a = config.fieldConfig.extensions) === null || _a === void 0 ? void 0 : _a.engram) === null || _b === void 0 ? void 0 : _b.config.authorize;
            ...
            if (!((_c = config.schemaConfig.plugins) === null || _c === void 0 ? void 0 : _c.find((p) => p.config.name === 'EngramAuthorize'))) {
                if (!hasWarned) {
                    console.warn('The GraphQL Engram "authorize" feature has been moved to a plugin, add [fieldAuthorizePlugin()] to your makeSchema plugin config to remove this warning.');
                    hasWarned = true;
                }
            }
            ...
            return function (root, args, ctx, info, next) {
                ...
                return plugin_1.plugin.completeValue(toComplete, (authResult) => {
                    ...
                    finalFormatError(new Error(`Engram authorize for ${parentTypeName}.${fieldName} Expected a boolean or Error, saw ${authResult}`));
                }, (err) => {

...
class SchemaBuilder {
    constructor(config) {
        ...
        this.addType = (typeDef) => {
            var _a;
            if (wrapping_1.isEngramDynamicInputMethod(typeDef)) {
                this.dynamicInputFields[typeDef.name] = typeDef;
                return;
            }
            if (wrapping_1.isEngramDynamicOutputMethod(typeDef)) {
                this.dynamicOutputFields[typeDef.name] = typeDef;
                return;
            }
            if (wrapping_1.isEngramDynamicOutputProperty(typeDef)) {
            ...
            if (wrapping_1.isEngramExtendTypeDef(typeDef)) {
            ...
            if (wrapping_1.isEngramExtendInputTypeDef(typeDef)) {
            ...
            if (wrapping_1.isEngramScalarTypeDef(typeDef) && typeDef.value.asEngramMethod) {
            ...
            if (wrapping_1.isEngramInputObjectTypeDef(typeDef)) {
                this.typesToWalk.push({ type: 'input', value: typeDef.value });
            }
            if (wrapping_1.isEngramObjectTypeDef(typeDef)) {
                this.typesToWalk.push({ type: 'object', value: typeDef.value });
            }
            if (wrapping_1.isEngramInterfaceTypeDef(typeDef)) {
    ...
    addTypes(types) {
        ...
        if (wrapping_1.isEngramPlugin(types)) {
            if (!((_a = this.config.plugins) === null || _a === void 0 ? void 0 : _a.includes(types))) {
                throw new Error(`Engram plugin ${types.config.name} was seen in the "types" config, but should instead be provided to the "plugins" array.`);
            }
            return;
        }
        if (wrapping_1.isEngramNamedTypeDef(types) ||
            wrapping_1.isEngramExtendTypeDef(types) ||
            wrapping_1.isEngramExtendInputTypeDef(types) ||
            graphql_1.isNamedType(types) ||
            wrapping_1.isEngramDynamicInputMethod(types) ||
            wrapping_1.isEngramDynamicOutputMethod(types) ||
            wrapping_1.isEngramDynamicOutputProperty(types)) {
            this.addType(types);
        }
    ...
    beforeWalkTypes() {
        this.plugins.forEach((obj, i) => {
            if (!wrapping_1.isEngramPlugin(obj)) {
    ...
    createSchemaExtension() {
        this._schemaExtension = new extensions_1.EngramSchemaExtension({
            ...this.config,
            dynamicFields: {
    ...
    buildInputObjectType(config) {
        ...
        const inputObjectTypeConfig = {
            ...
            extensions: {
                engram: new extensions_1.EngramInputObjectTypeExtension(config),
            },
        };
    ...
    buildObjectType(config) {
        ...
        const objectTypeConfig = {
            ...
            extensions: {
                engram: new extensions_1.EngramObjectTypeExtension(config),
            },
        };
    ...
    buildInterfaceType(config) {
        ...
        const interfaceTypeConfig = {
            ...
            extensions: {
                engram: new extensions_1.EngramInterfaceTypeExtension(config),
            },
        };
    ...
    buildEnumType(config) {
        ...
        if (!Object.keys(values).length) {
            throw new Error(`GraphQL Engram: Enum ${config.name} must have at least one member`);
        }
        if (config.rootTyping) {
            this.rootTypings[config.name] = config.rootTyping;
        }
    ...
    buildUnionMembers(unionName, members) {
        ...
        if (!unionMembers.length) {
            throw new Error(`GraphQL Engram: Union ${unionName} must have at least one member type`);
        }
        return unionMembers;
    }
    ...
    buildOutputField(fieldConfig, typeConfig) {
        ...
        const fieldExtension = new extensions_1.EngramFieldExtension(fieldConfig);
    ...
    inputNonNull(typeDef, field) {
        ...
        const { name, nonNullDefaults = {} } = ((_b = (_a = typeDef.extensions) === null || _a === void 0 ? void 0 : _a.engram) === null || _b === void 0 ? void 0 : _b.config) || {};
    ...
    outputNonNull(typeDef, field) {
        ...
        const { nonNullDefaults = {} } = (_c = (_b = (_a = typeDef.extensions) === null || _a === void 0 ? void 0 : _a.engram) === null || _b === void 0 ? void 0 : _b.config) !== null && _c !== void 0 ? _c : {};
        if (typeof nullable !== 'undefined') {
            return !nullable;
        }
        // Non-Null by default
    ...
    getObjectType(name) {
        if (wrapping_1.isEngramNamedTypeDef(name)) {
    ...
    getOrBuildType(name, fromObject = false) {
        ...
        if (this.buildingTypes.has(name)) {
            /* istanbul ignore next */
            throw new Error(`GraphQL Engram: Circular dependency detected, while building types ${Array.from(this.buildingTypes)}`);
        }
        const pendingType = this.pendingTypeMap[name];
        if (wrapping_1.isEngramNamedTypeDef(pendingType)) {
            this.buildingTypes.add(pendingType.name);
            if (wrapping_1.isEngramObjectTypeDef(pendingType)) {
                return this.buildObjectType(pendingType.value);
            }
            else if (wrapping_1.isEngramInterfaceTypeDef(pendingType)) {
                return this.buildInterfaceType(pendingType.value);
            }
            else if (wrapping_1.isEngramEnumTypeDef(pendingType)) {
                return this.buildEnumType(pendingType.value);
            }
            else if (wrapping_1.isEngramScalarTypeDef(pendingType)) {
                return this.buildScalarType(pendingType.value);
            }
            else if (wrapping_1.isEngramInputObjectTypeDef(pendingType)) {
                return this.buildInputObjectType(pendingType.value);
            }
            else if (wrapping_1.isEngramUnionTypeDef(pendingType)) {
    ...
    maybeTraverseOutputFieldType(type) {
        ...
        if (args) {
            utils_1.eachObj(args, (val) => {
                const t = wrapping_1.isEngramArgDef(val) ? val.value.type : val;
                if (typeof t !== 'string') {
                    this.addType(t);
                }
            });
...
async function generateSchema(config) {
    ...
    const typegenConfig = typegenUtils_1.resolveTypegenConfig(finalConfig);
    utils_1.assertNoMissingTypes(schema, missingTypes);
    await new typegenMetadata_1.TypegenMetadata(typegenConfig).generateArtifacts(schema);
    return schema;
}
 ...
 */
generateSchema.withArtifacts = async (config, typeFilePath) => {
    const { schema, missingTypes, finalConfig } = makeSchemaInternal(config);
    const typegenConfig = typegenUtils_1.resolveTypegenConfig(finalConfig);
    utils_1.assertNoMissingTypes(schema, missingTypes);
    const { schemaTypes, tsTypes } = await new typegenMetadata_1.TypegenMetadata(typegenConfig).generateArtifactContents(schema, typeFilePath);
...
function invariantGuard(val) {
    ...
    if (Boolean(val) === false) {
        throw new Error('Engram Error: This should never happen, ' +
...
function normalizeArg(argVal) {
    if (wrapping_1.isEngramArgDef(argVal)) {
        return argVal;
    }

...
import { AllEngramOutputTypeDefs } from '../definitions/wrapping';
...
export interface ConnectionPluginConfig {
	...
	typePrefix?: string;
	/**
	 * The path to the @engram/schema package. Needed for typegen.
	 *
...
> = {
	...
	| {
			 ...
			 * GraphQL Relay JS, and only use Engram for the construction and type-safety:
			 *
...
export const connectionPlugin = (
	...
	return plugin({
		...
		fieldDefTypes: [
			printedGenTypingImport({
				module:
					connectionPluginConfig?.engramSchemaImportId ?? getOwnPackage().name,
		...
		onInstall(b) {
			...
			const {
				...
				engramFieldName = 'connectionField',
			} = pluginConfig;
...
const assertCorrectConfig = (
	...
	if (
		...
		console.error(
			new Error(
				`Engram Connection Plugin: Missing nodes or resolve property for ${typeName}.${fieldName}`
			)
		);
	}
	eachObj(pluginConfig.extendConnection || {}, (val, key) => {
		if (typeof fieldConfig[key] !== 'function') {
			console.error(
				new Error(
					`Engram Connection Plugin: Missing ${key} resolver property for ${typeName}.${fieldName}`
				)
			);
		}
	});
	eachObj(pluginConfig.extendEdge || {}, (val, key) => {
		if (
			...
			console.error(
				new Error(
					`Engram Connection Plugin: Missing edgeFields.${key} resolver property for ${typeName}.${fieldName}`
				)
			);
		}
	});

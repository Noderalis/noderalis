...
export const fieldAuthorizePlugin = (
	...
	return plugin({
		...
		onCreateFieldResolver(config) {
			const authorize = config.fieldConfig.extensions?.engram?.config.authorize;
			...
			if (
				!config.schemaConfig.plugins?.find(
					(p) => p.config.name === 'EngramAuthorize'
				)
			) {
				if (!hasWarned) {
					console.warn(
						'The GraphQL Engram "authorize" feature has been moved to a plugin, add [fieldAuthorizePlugin()] to your makeSchema plugin config to remove this warning.'
					);
					hasWarned = true;
				}
			}
			...
			return function (root, args, ctx, info, next) {
				...
				return plugin.completeValue(
					...
					(authResult) => {
						...
						finalFormatError(
							new Error(
								`Engram authorize for ${parentTypeName}.${fieldName} Expected a boolean or Error, saw ${authResult}`
							)
						);
					},
					(err) => {

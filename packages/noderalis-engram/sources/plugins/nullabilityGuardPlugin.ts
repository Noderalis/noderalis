...
import { EngramGraphQLNamedType } from '../extensions';
...
export const nullabilityGuardPlugin = (
	...
	return plugin({
		...
		onCreateFieldResolver(
			...
			if (config.fieldConfig.extensions?.engram?.config.skipNullGuard) {
				return;
			}
		...
		onAfterBuild(schema) {
			Object.keys(schema.getTypeMap()).forEach((typeName) => {
				const type = schema.getType(typeName) as EngramGraphQLNamedType;
			...
			if (pluginConfig.fallbackValues) {
				Object.keys(pluginConfig.fallbackValues).forEach((name) => {
					const type = schema.getType(name) as EngramGraphQLNamedType;
					if (!type) {

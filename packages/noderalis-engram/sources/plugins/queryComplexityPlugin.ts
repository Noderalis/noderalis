...
export type QueryComplexity<
	TypeName extends string,
	FieldName extends string
> = number | QueryComplexityEstimator<TypeName, FieldName>;
...
export const queryComplexityPlugin = () => {
    ...
    `,
		...
		onCreateFieldResolver(config) {
			...
			const complexity =
				config.fieldConfig.extensions?.engram?.config.complexity;
			...
			if (typeof complexity !== 'number' && typeof complexity !== 'function') {
				...
				const fieldName = config.fieldConfig.extensions?.engram?.config.name;
				console.error(
					new Error(

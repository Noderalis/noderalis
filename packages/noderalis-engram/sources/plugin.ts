...
export type CreateFieldResolverInfo<FieldExt = any, TypeExt = any> = {
	...
	fieldConfig: Omit<EngramGraphQLFieldConfig, 'resolve' | 'extensions'> & {
		extensions?: Maybe<{ engram?: { config: FieldExt } }>;
	};
	/**
	 ...
	 */
	parentTypeConfig: (
		| Omit<EngramGraphQLObjectTypeConfig, 'fields' | 'extensions'>
		| Omit<EngramGraphQLInterfaceTypeConfig, 'fields' | 'extensions'>
	) & {
		extensions?: Maybe<{ engram?: { config: TypeExt } }>;
	};
	/**
...
withEngramSymbol(EngramPlugin, EngramTypes.Plugin);
...
export function plugin(config: PluginConfig) {
	...
	return new EngramPlugin(config);
}

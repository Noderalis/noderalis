...
export function typegenAutoConfig(options: TypegenAutoConfigOptions) {
	return async (
		...
		const typeSources = await Promise.all(
			options.sources.map(async (source) => {
				...
				if (
					...
					return console.warn(
						`Engram Schema Typegen: Expected module ${pathOrModule} to be an absolute path to a TypeScript module, skipping.`
					);
				}
				...
				try {
					...
					if (
						...
						console.error(
							`GraphQL Engram: Unable to find file or module ${pathOrModule}, skipping`
						);
					} else {
				...
				if (allImportsMap[alias] && allImportsMap[alias] !== importPath) {
					return console.warn(
						`Engram Schema Typegen: Cannot have multiple type sources ${importsMap[alias]} and ${pathOrModule} with the same alias ${alias}, skipping`
					);
				}
		...
		const typegenInfo = {
			...
			imports,
			contextType,
			engramSchemaImportId: getOwnPackage().name,
		};
